...
class Agent {
  ...
  minVal(state, depth, alpha, beta) {
    ...
    for (let i = 0; i < actions.length; i++) {
      ...
      minimaxVal = Math.min(minimaxVal, this.maxVal(newState, depth + 1, alpha, beta));
  ...
  maxVal(state, depth, alpha, beta) {
    ...
    for (let i = 0; i < actions.length; i++) {
      let action = actions[i];
      let newState = state.transition(action);
      minimaxVal = Math.max(minimaxVal, this.minVal(newState, depth + 1, alpha, beta));
